...
| reduced regressions consistent | Thm 1 | the corrections' limits | estimated, unmeasured rates | `test_reduced_regressions.py` shows a **saturated** learner recovers them exactly on the exact law; that is consistency at one learner on one law and not a rate. [E2's dispatch](investigation-log.md#what-the-e2-dispatch-measured) was the run built to say whether `glm` here is where the remainder lives, and it **returned no branch**: gate B failed in three cells of four, so their paired differences are `unresolved` however large, and the one gate-passing cell read `moved`. What it did establish is about the *reference* — a coarser rung is the better estimate of at least one reduced regression, and which one differs by cell. [E2R](../roadmap.md#what-e2rs-instrument-landed) is the repaired dispatch and its instrument has landed — the rung selected rather than shipped, certified on a block that did not choose it — so this row's question now has a run built for it and not yet made | **unverified**. The status does not move on a comparison that could not be read, in either direction — the same rule an instrument landing is held to two rows up |
