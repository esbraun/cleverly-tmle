...
| categorical treatment nodes, static and dynamic regimens | `tests/discrete_law_longitudinal_multivalue.py`, `tests/unit/test_influence_gateaux_longitudinal_multivalue.py` | nonzero quadratic remainder and a third-arm selection mutation that rejects the binary complement; string-label end outcome, censoring, MSM, survival, and competing-risk fits in `tests/e2e/test_ltmle_multivalue.py` | source audit of R `ltmle`, `tmle3`, and the Poulos companion repository, with snapshots recorded in `docs/references.md` | deterministic categorical regimens only; no stochastic categorical policies or continuous longitudinal dose |
