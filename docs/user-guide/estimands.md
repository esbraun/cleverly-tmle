...
ate = study.estimate(ATE(reference=0), random_state=3)
levels = study.estimate(CounterfactualMean(), random_state=3)
relative = study.estimate(RiskRatio(reference=0), random_state=3)
```
...
result = study.estimate(RegimeContrast(plans, reference="treat none"), random_state=3)
```
...
result = study.estimate(IncrementalEffect(tilts), random_state=3)
```
