...
inspect the fit before it could refuse. Examples include an E-value on a fit without a contrast
and a missingness tilt on a fit without missing outcomes. Such a row starts with `refused on inspection:` and names
