...
- `repeats` repeats the outer split and reports the median of the draws. Default 1. The value must
  be at least 1, and a value above 1 requires `enabled=True`. `CrossFitting` raises
  `MethodConfigurationError` for either violation when you construct it. A repeated fit refuses a
  simultaneous band, because the band needs the joint influence curves of one draw. Pass
  `simultaneous=False` to a repeated fit.
