...
| a confidence interval, a p-value, or a standard error on a saved cross-fitted result whose outer folds read the treatment | [RM31](../roadmap.md#rm31-inference-status-of-a-saved-stratified-cross-fitted-result). A `TMLE` or `DRTMLE` result with `stratify_folds="treatment"` or `"treatment+outcome"` takes `"stratified_fold_plugin"` when it loads. An `LTMLE` result with more than one fold and no `Folds.origin` takes it too. Releases 0.1.0 and 0.1.1 drew both splits by default, and new fits refuse them. The point estimate and plug-in diagnostic remain; saved bands and assessment answers are dropped. A new fit with unstratified folds, which is the default, reports an interval |
