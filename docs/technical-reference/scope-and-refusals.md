...
| a confidence interval, a p-value, or a standard error on a cross-fitted `TMLE` or `DRTMLE` fit with `id=` whose clusters hold different numbers of rows, or different weight mass on a weighted fit, including `cv_evaluation=True` | [clusters](inference.md#clusters). The fit takes the `"unequal_cluster_plugin"` status. The [grouped folds](cv-tmle.md#grouped-folds) argument needs equal cluster sizes, because only then does the row-weighted target equal the cluster-weighted target. A weighted fit targets the weight-weighted mean, so the size of a cluster is its row count and its weight mass. The point estimate remains, and `plugin_std_error` and `plugin_interval` report the retained diagnostic. The same clusters fitted in sample keep the interval when there are 40 or more, in the fit and in each reported stratum. [F22](../roadmap.md#f22-grouped-cross-fitting-beyond-point-treatment-tmle) reopens it |
| a confidence interval, a p-value, or a standard error on a `TMLE` or `DRTMLE` fit with `id=` and fewer than 40 clusters in the fit or in one reported baseline stratum, in sample or cross-fitted | [clusters](inference.md#clusters). The fit takes the `"few_cluster_plugin"` status. A fit has one status, so one such stratum withholds the interval of every estimate. The package uses a normal reference. Nugent et al. (2024), Section 2.2, recommend a $t$ reference below 40 clusters, and no registered study covers few clusters. The point estimate remains, and `plugin_std_error` and `plugin_interval` report the retained diagnostic. [F22](../roadmap.md#f22-grouped-cross-fitting-beyond-point-treatment-tmle) reopens it |
| a contrast, a simultaneous band, any E-value branch, `variable_importance()`, or `tipping_gamma(use_ci=True)` on the fits of the five rows above | [inference status](inference.md#inference-status). Each one is a confidence statement, or reads an interval that these fits do not supply. `variable_importance()` refuses at its entry point, because it adjusts one p-value per candidate. The E-value capability reports `unavailable` rather than raising inside the computation. `truncation_curve()`, `missingness_tilt()`, and the default `tipping_gamma()` still run, and the two frames rename their three spread columns |
...
| --- | --- |
| a `Stochastic` regime whose density came from the estimated mechanism. **Not refused yet** | `g*` becomes a functional of `P`, so the influence curve carries a term for the pathwise derivative through `ghat` that a regime's curve does not have. The reported standard error is too **small**. No code checks that a `density_fn` is known, so such a regime fits with no message. [RM25](../roadmap.md#rm25-declared-stochastic-regime-densities) tracks a declaration and a refusal |
...
| an MSM `design` that reads a sample statistic, such as a covariate centred at its sample mean. **Not refused yet** | the same argument for the design. The projection becomes a functional of `P` through the design, and a term goes missing from the influence curve. On one exact law, the reported standard error of the intercept reads 0.893 of the exact one. No code checks that a `design` is known. [RM27](../roadmap.md#rm27-declared-msm-design-functions) tracks a declaration and a refusal |
