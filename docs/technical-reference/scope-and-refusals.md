...
| a contrast, a simultaneous band, or any E-value branch on the same three paths | [collaborative TMLE](collaborative-tmle.md). Each one is a confidence statement, or reads an interval that these paths do not supply. The E-value capability reports `unavailable` rather than raising inside the computation |
