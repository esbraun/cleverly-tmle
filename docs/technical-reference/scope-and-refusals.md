...
| a confidence interval, a p-value, or a standard error on a saved cross-fitted result whose outer folds read the treatment | [RM31](../roadmap.md#rm31-inference-status-of-a-saved-stratified-cross-fitted-result). A `TMLE` or `DRTMLE` result of a discrete treatment with `stratify_folds="treatment"` or `"treatment+outcome"` takes `"stratified_fold_plugin"` when it loads. An `LTMLE` result with more than one fold and no `Folds.origin` takes it too. Releases 0.1.0 and 0.1.1 drew both splits by default, and new fits refuse them. The point estimate and plug-in diagnostic remain. Loading drops the saved bands and assessment answers. A new cross-fitted fit defaults to unstratified folds. It reports an interval only when its other inference rules permit one |
| a confidence interval, a p-value, or a standard error on a saved cross-fitted result of a continuous outcome with `q_bounds=None` | [RM33](../roadmap.md#rm33-inference-status-of-a-saved-undeclared-scale-cross-fitted-result). A continuous-dose `TMLE` result takes `"undeclared_scale_plugin"` when it loads, and so does a copied `TMLE` or `DRTMLE` estimator on that scale. Releases 0.1.0 and 0.1.1 took that outcome scale from every observed outcome, held-out rows included, and new fits refuse it. A saved cross-fitted discrete-treatment result takes `"stratified_fold_plugin"` first ([RM31](../roadmap.md#rm31-inference-status-of-a-saved-stratified-cross-fitted-result)). The point estimate and plug-in diagnostic remain. Loading drops the saved bands and assessment answers. A new cross-fitted fit that declares `q_bounds`, or an in-sample fit, reports an interval only when its other inference rules permit one |
...
| a contrast, a simultaneous band, any E-value branch, `variable_importance()`, or `tipping_gamma(use_ci=True)` on the fits of the eight rows above | [inference status](inference.md#inference-status). Each one is a confidence statement, or reads an interval that these fits do not supply. `variable_importance()` refuses at its entry point, because it adjusts one p-value per candidate. The E-value capability reports `unavailable` rather than raising inside the computation. `truncation_curve()`, `missingness_tilt()`, and the default `tipping_gamma()` still run. On a point-treatment fit the two frames rename their three spread columns. The longitudinal `truncation_curve()` publishes no spread. `LongitudinalResult.curve()` and `incidence_total()` rename their spread columns |
| a confidence interval, a p-value, or a standard error on a `ParameterEstimate` that release 0.1.0 or 0.1.1 saved apart from its result | [RM34](../roadmap.md#rm34-inference-status-of-a-saved-estimate-outside-its-result). The estimate takes `"unrecorded_status_plugin"` when it loads. Its state records no status, and it holds no configuration to read one from. The estimate inside a `VariableImportanceEntry` takes the status, and the entry reads `adjusted_pvalue` as `None`. `CVTargeting.inference` reads the status of its estimates, so a fold-level report saved alone refuses `std_error`. A contrast of such estimates inherits the status, and a simultaneous band refuses it. The point estimate and plug-in diagnostic remain. A whole saved result gives each estimate the status of its own configuration, so load the whole result, or fit again |
