...
| any E-value request on a fit with an intermediate variable | [the E-value paths](validation-methods.md#e-value). No read source derives an E-value for a controlled direct effect. The E-value bounds the confounding of one exposure-outcome relation. A controlled direct effect also assumes no unmeasured confounding of the intermediate variable and the outcome. The capability row reports `unavailable` before any computation. [F25](../roadmap.md#f25-e-value-for-a-controlled-direct-effect) reopens it |
