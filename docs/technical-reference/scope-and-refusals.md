...
| an MSM `design` that reads a sample statistic, such as a covariate centred at its sample mean | the same argument for the design. The projection becomes a functional of `P` through the design, and a term goes missing from the influence curve. On one exact law, the reported standard error of the intercept reads 0.893 of the exact one (`tests/unit/test_msm_design_declaration.py`). `MSM` refuses `design_kind="estimated"` and a `design=` callable with no declaration. A design that `MSM.linear` builds needs no declaration. A false `"known"` declaration still fits because code cannot inspect a closure. [MSM projections](msm-projections.md#variations) gives the declaration, and [RM27](../roadmap.md#rm27-declared-msm-design-functions) records the witness |
