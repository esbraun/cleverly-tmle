...
| simulated common-cause sensitivity | supported for binary and continuous treatment; waiting on published theory for multi-arm treatment | The binary surface accepts marginal and baseline-stratum arm means, ATE, and ratios under ordinary TMLE and C-TMLE. Ordinary TMLE also accepts ATT and ATC with perturbed group membership. Complete-outcome DR-TMLE supports marginal arm means, ATE, and ratios only. The continuous surface accepts marginal and baseline-stratum modified-policy means and contrasts under ordinary TMLE. Every listed row accepts fixed probability weights. The zero-delta policy mean remains refused. See the [population contract and refusal table](validation-methods.md#simulated-common-cause-stress-surface) |
