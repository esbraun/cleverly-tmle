...
| a confidence interval, a p-value, or a standard error on a saved cross-fitted result of a continuous outcome with `q_bounds=None` | [RM33](../roadmap.md#rm33-inference-status-of-a-saved-unbounded-scale-cross-fitted-result). A `TMLE` or `DRTMLE` result takes `"undeclared_scale_plugin"` when it loads. Releases 0.1.0 and 0.1.1 took that outcome scale from every observed outcome, held-out rows included, and new fits refuse it. RM31 reaches a saved discrete treatment first, so this row reaches a saved continuous dose. The point estimate and plug-in diagnostic remain. Loading drops the saved bands and assessment answers. A new cross-fitted fit that declares `q_bounds`, or an in-sample fit, reports an interval only when its other inference rules permit one |
