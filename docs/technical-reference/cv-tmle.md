...
| a copied or modified estimator carrying a refused policy | `CapabilityError`, a subclass of `ValueError`, before any learner of a fit or a refit. A result that holds such an estimator reads `refit_nuisances` false, with the code `point_replay_refit_configuration` | the reason above, then "This fit was configured under a fold policy this version refuses, which a copied or modified estimator can still carry" |
...
| a cross-fitted continuous outcome with `q_bounds=None` | `CapabilityError` before any learner of a fit or a refit. A copied or modified `TMLE` or `DRTMLE` estimator on that scale meets the same refusal at its refit | "Declare the known outcome support (Targeting(q_bounds=(lower, upper))). Without a known finite support, fit in sample with cross_fit=False on the engine (CrossFitting(enabled=False))" |
...
| a refused policy reaches no learner, at construction and on a modified estimator | `TestAPolicyThisVersionRefusesNeverReachesALearner` |
...
| `result.split_plan` of an in-sample fit | nothing | refuses it |
