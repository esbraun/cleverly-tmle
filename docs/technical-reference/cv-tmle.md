...
| outcome scale under cross-fitting | a declared `q_bounds`. A cross-fitted continuous outcome with `q_bounds=None` is refused before nuisance fitting. A restored continuous-dose result, or a copied estimator, on that scale takes `"undeclared_scale_plugin"` | `TMLE._refuse_unbounded_cross_fitted_scale`, `TMLE._saved_scale_status`, `LTMLE._refuse_cross_fitted_design` |
...
| a cross-fitted continuous outcome with `q_bounds=None` | `CapabilityError`, before any learner of a fit or a refit. A restored result or a copied estimator on that scale also takes `"undeclared_scale_plugin"`, so `ci`, `pvalue`, and `std_error` refuse ([RM33](../roadmap.md#rm33-inference-status-of-a-saved-unbounded-scale-cross-fitted-result)) | "Declare the known outcome support (Targeting(q_bounds=(lower, upper))). Without a known finite support, fit in sample with cross_fit=False on the engine (CrossFitting(enabled=False))" |
