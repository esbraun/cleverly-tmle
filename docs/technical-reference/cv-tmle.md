...
| outcome scale under cross-fitting | a declared `q_bounds`. A cross-fitted continuous outcome with `q_bounds=None` is refused before nuisance fitting. A restored result on that scale takes `"undeclared_scale_plugin"` | `TMLE._refuse_unbounded_cross_fitted_scale`, `TMLE._saved_scale_status`, `LTMLE._refuse_cross_fitted_design` |
