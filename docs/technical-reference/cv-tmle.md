...
| longitudinal folds | `random_partition` draws them, and `Folds.origin` records the draw. There are no first-node treatment strata. A restored result with more than one fold and no origin gets `"stratified_fold_plugin"` under the saved-fold rule, subject to earlier statuses | `LTMLE._folds`, `_saved_split_status` |
...
| saved point-treatment outcome scale | A restored cross-fitted continuous-dose `TMLE` result with a continuous outcome and `q_bounds=None` takes `"undeclared_scale_plugin"`. A copied `TMLE` or `DRTMLE` estimator on that scale takes the same status. A legacy cross-fitted discrete-treatment result takes `"stratified_fold_plugin"` first | `TMLE._saved_scale_status` |
| saved longitudinal outcome scale | `LTMLE` has no saved-scale rule. A legacy cross-fitted result with `q_bounds=None` gets `"stratified_fold_plugin"` from its unrecorded fold origin, subject to earlier statuses | `_saved_split_status` |
...
| a cross-fitted continuous outcome with `q_bounds=None` | `CapabilityError` before any learner of a fit or a refit. A restored cross-fitted continuous-dose `TMLE` result on that scale takes `"undeclared_scale_plugin"`, so its `ci`, `pvalue`, and `std_error` refuse ([RM33](../roadmap.md#rm33-inference-status-of-a-saved-undeclared-scale-cross-fitted-result)). A copied `TMLE` or `DRTMLE` estimator on that scale takes the same status | "Declare the known outcome support (Targeting(q_bounds=(lower, upper))). Without a known finite support, fit in sample with cross_fit=False on the engine (CrossFitting(enabled=False))" |
...
| a cross-fitted longitudinal continuous outcome with `q_bounds=None` | `LongitudinalError`. A legacy saved result on that scale gets `"stratified_fold_plugin"` under its saved-fold rule, subject to earlier statuses. `LTMLE` has no saved-scale rule | "Declare the known outcome support (Targeting(q_bounds=(lower, upper))), or fit in sample (CrossFitting(enabled=False), or n_folds=1 on the engine)." |
