...
| a restored result or a copied estimator carrying a refused policy | `CapabilityError`, a subclass of `ValueError`, before any learner of a fit or a refit. A restored result reads `refit_nuisances` false, with the code `point_replay_refit_configuration`. A restored cross-fitted result under the policy also takes `"stratified_fold_plugin"`, so `ci`, `pvalue`, and `std_error` refuse ([RM31](../roadmap.md#rm31-inference-status-of-a-saved-stratified-cross-fitted-result)) | the reason above, then "This fit was configured under a fold policy this version refuses, which a restored result or a copied estimator can still carry" |
