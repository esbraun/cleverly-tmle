...
| `ey_obs` | `tests/discrete_law.py`, `tests/discrete_law_mar.py` | `tests/unit/test_influence_gateaux.py`, `tests/unit/test_influence_gateaux_natural_course_mar.py` | `tests/unit/test_remainder_natural_course_mar.py` for missing outcomes | the complete-data curve is `w(Y - E_w[Y])`; at `pi == 1` the MAR estimate and curve reduce exactly to that empirical result. Respondent-mask, all-row plug-in, response-score mutation, and fail-if-fit treatment-learner controls distinguish the MAR construction | the MAR interval covers one iid scalar ordinary-TMLE configuration; cross-fitting, repeats, weighting, clusters, strata, joint targeting, bootstrap, and the attributable targets remain refused |
