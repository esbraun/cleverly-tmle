...
| `ey_obs` | `tests/discrete_law.py`, `tests/discrete_law_mar.py` | `tests/unit/test_influence_gateaux.py`, `tests/unit/test_influence_gateaux_natural_course_mar.py` | `tests/unit/test_remainder_natural_course_mar.py` for missing outcomes | the complete-data curve is `w(Y - E_w[Y])`; at `pi == 1` the natural-course construction itself returns that estimate and that curve, which the test asserts against the construction rather than through a fit, because a frame with no missing outcome takes the complete-data branch. Respondent-mask, all-row plug-in, response-score mutation, and fail-if-fit treatment-learner controls distinguish the MAR construction | the MAR interval covers one iid scalar ordinary-TMLE configuration; cross-fitting, repeats, weighting, clusters, strata, `intermediate=`, joint targeting, bootstrap, and the attributable targets remain refused |
