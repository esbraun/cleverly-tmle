...
| `"undeclared_function_plugin"` | a restored `TMLE` result whose `Rule`, user-written `Intervention`, `Stochastic` density, or written MSM design or weight is not declared `"known"`. A restored `LTMLE` result takes it when a callable node of its regimen is not declared `"known"`. New fits refuse such a function. A new fit with each function declared `"known"` reports an interval. [RM28](../roadmap.md#rm28-declared-densities-of-user-written-interventions) gives the load rule | raise `CapabilityError` with the reason of the status | `undeclared-function se` | [RM28](../roadmap.md#rm28-declared-densities-of-user-written-interventions) |
