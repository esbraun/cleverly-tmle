...
| `"stratified_fold_plugin"` | a saved cross-fitted `TMLE` or `DRTMLE` result with `stratify_folds="treatment"` or `"treatment+outcome"`, and a saved `LTMLE` result with more than one fold whose `Folds.origin` is `None`. Releases 0.1.0 and 0.1.1 drew both splits by default, and new fits refuse them. An in-sample result and a continuous-dose result keep `"influence_curve"`, because their split read no treatment. [RM31](../roadmap.md#rm31-inference-status-of-a-saved-stratified-cross-fitted-result) gives the load rule | raise `CapabilityError` with the reason of the status | `stratified-fold plug-in se` | [RM31](../roadmap.md#rm31-inference-status-of-a-saved-stratified-cross-fitted-result) |
...
stratified result of each path and checks its reports.
