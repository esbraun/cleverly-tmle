...
| `"undeclared_scale_plugin"` | a saved cross-fitted `TMLE` or `DRTMLE` result of a continuous outcome with `q_bounds=None`. Releases 0.1.0 and 0.1.1 took that outcome scale from every observed outcome, held-out rows included, and new fits refuse it. RM31 reaches a saved discrete treatment first, so this status reaches a saved continuous dose. A declared `q_bounds`, an in-sample result, and a binary outcome keep `"influence_curve"`. [RM33](../roadmap.md#rm33-inference-status-of-a-saved-unbounded-scale-cross-fitted-result) gives the load rule | raise `CapabilityError` with the reason of the status | `undeclared-scale plug-in se` | [RM33](../roadmap.md#rm33-inference-status-of-a-saved-unbounded-scale-cross-fitted-result) |
