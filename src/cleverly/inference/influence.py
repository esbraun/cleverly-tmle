...
def stamp_inference(
    ...
    dict of str to ParameterEstimate
        ``estimates`` in the same order. Under ``"influence_curve"``, an estimate that
        loaded under ``"unrecorded_status_plugin"`` is a copy that declares
        ``"influence_curve"``, and every other estimate is returned as it was given. A
        recorded status is kept, so a current artifact loads as it was saved. Under any
        other status each estimate is a copy declaring ``status``. Every array and
        number is unchanged.
    """
    if supplies_inference(status):
        return {
            name: replace(estimate, inference=status)
            if estimate.inference == UNRECORDED_STATUS
            else estimate
...
def carries_status(
    reports: Iterable[Mapping[str, ParameterEstimate]], status: InferenceStatus
) -> bool:
    ...
    Parameters
    ----------
    reports : iterable of mapping of str to ParameterEstimate
        Every report of the result, keyed by estimand.
    status : str
        The status the result's configuration gives. One of :data:`InferenceStatus`.

    Returns
    -------
    bool
        ``True`` when every estimate declares ``status`` and ``status`` is not
        ``"unrecorded_status_plugin"``.
    """
    return status != UNRECORDED_STATUS and all(
...
class ParameterEstimate:
    ...
    inference : str
        ...
        :attr:`plugin_interval` report the retained diagnostic. The default applies to a
        constructor call. A pickle whose state has no ``inference`` key loads under
        ``"unrecorded_status_plugin"``.
...
def median_estimates(
    ...
    r"""Combine repeated cross-fitting estimates by their median.
    ...
    .. math::
        ...
        \widehat\sigma_r^2 + (\widehat\psi_r - \widetilde\psi)^2\right\}.

    This rule includes within-draw uncertainty and between-draw displacement. For an odd
    repeat count, the reported marginal influence curve comes from the draw at the median
    point. For an even count, it is the mean of the two central draws' curves. The curve
    supports marginal diagnostics. The adjusted variance, not the curve's empirical
    variance, supplies the interval.

    Parameters
    ----------
    per_repeat : sequence of mappings
        One estimate mapping per draw. A one-element sequence returns its input unchanged.

    Returns
    -------
    dict
        Median-combined estimates for names present in every draw.
    """
    if not per_repeat:
        raise ValueError("median_estimates needs at least one repeat")
    if len(per_repeat) == 1:
    ...
    if dropped:
        # Silence here would report a median over a subset of the draws under the same
        # name as a median over all of them.
        warnings.warn(
            ...
            "units in the conditioning arm -- so it is omitted from the median report "
            "rather than combined over fewer draws than the rest.",
            UserWarning,
            stacklevel=2,
        )
    ...
    for name in shared:
        ...
        middle = len(parts) // 2
        central = order[middle : middle + 1] if len(parts) % 2 else order[middle - 1 : middle + 1]
        ...
        variance = float(
            np.median(
                [
                    part.variance + (point - median_point) ** 2
                    for part, point in zip(parts, points, strict=True)
                ]
            )
        )
        out[name] = ParameterEstimate(
            name=name,
            psi=psi,
            influence_curve=np.asarray(influence_curve, dtype=float),
            variance=variance,
            n=parts[0].n,
            n_clusters=parts[0].n_clusters,
            scale=scale,
            alpha=parts[0].alpha,
            log_psi=log_psi,
