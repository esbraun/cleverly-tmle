...
fourteen-column table :mod:`tests.studies.evidence.schema` declares.  That ending is written
out twelve times.  It has already drifted.  ``tests/canonical/tmle3/run_tmle3.R`` carries a
memory cap and a malformed-worker check added after a run lost 86 of 3,200 replications while
reporting success; ``tests/canonical/drtmle/run_drtmle.R`` has neither, and the two runners
this repository added most recently carry the cap without the reasoning that produced it.

The copies are not an oversight that can simply be deleted.
``test_method_evidence.py::test_the_manifest_hashes_every_published_result`` asserts the exact
sha256 of every reference source, so changing a runner byte forces a re-run of that study's R
container -- hours of it, per study.  Collapsing them is a regeneration, not a refactor, and
it is sequenced as one.

What this module does is make the duplication safe to leave in the meantime, on the same terms
as :mod:`tests.unit.test_property_law_parity`.  It does not compare the scripts as text, which
would only assert that nobody had improved one of them.  It asserts the three things a copy
cannot quietly lose:
...
#: The ``data.frame(`` that builds a published row, and the ``name = `` keys inside it.  The
#: block is found by the column only a published row carries rather than by position, so a
#: runner that grows a second frame is still read at the right one.
...
class TestTheSharedPublishedContract:
    def test_the_published_row_is_the_declared_schema(self, runner: Path) -> None:
        keys = published_keys(runner.read_text(encoding="utf-8"))
    ...
    def test_the_runner_takes_the_three_paths_the_driver_passes(self, runner: Path) -> None:
        ...
        assert "if (length(args) != 3) stop(" in source, (
    ...
    def test_the_runner_checks_what_a_worker_returned(self, runner: Path) -> None:
        source = runner.read_text(encoding="utf-8")
        assert re.search(r"\binherits\b|\bis\.data\.frame\b", source), (
