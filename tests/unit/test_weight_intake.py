...
def test_constructors_preserve_normalization_and_weight_metadata(weights, alias) -> None:
    frame = _frame(weights)
    options = {"weights_type": alias, "weights_estimated": True}
    if weights is not None:
    ...
    assert point.weight_spec.estimated
...
def _emit_warning(emitter) -> None:
    emitter(np.array([1000.0] + [1.0] * 19), "mass")
...
def test_public_warning_helpers_keep_their_calling_convention(emitter) -> None:
    assert list(inspect.signature(emitter).parameters) == ["weights", "name"]
    with pytest.warns(WeightingWarning) as caught:
