...
def _arguments(study: ModuleType, here: Path, reference: Reference | None) -> argparse.Namespace:
    ...
    if arguments.replicates != study.PRIMARY_REPLICATES and not arguments.skip_properties:
        # A run below the declared primary budget is a smoke run, and each property module
        # fixes its own budget, so the property phase would run the declared study in full
        # and show its verdicts before the declaration is committed.  RM22's smoke run did
        # exactly that.  Such a run writes schema-bearing placeholders instead.
        print(
