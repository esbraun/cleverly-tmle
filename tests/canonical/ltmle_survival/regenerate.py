"""Regenerate ordinary survival LTMLE comparison and property artifacts."""
...
from tests.studies.canonical_ltmle_survival import (
    CONFIGURATION,
    PRIMARY_N,
    PRIMARY_REPLICATES,
    REFERENCE_METADATA,
...
from tests.studies.ltmle_survival_properties import generate_property_rows, summarize_properties
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="cleverly-ltmle-survival-") as raw:
        ...
        if cached:
            ...
            if python["replicate"].nunique() != args.replicates:
        ...
        if args.skip_r:
            ...
            r = committed.loc[
                (committed["implementation"] == "ltmle")
    ...
    if args.skip_properties:
        committed_rows = HERE / "property-replicates.csv.gz"
        committed_summary = HERE / "properties.csv"
        if committed_rows.exists() and committed_summary.exists():
            for name in ("property-replicates.csv.gz", "properties.csv"):
                committed = HERE / name
                if paths[name].resolve() != committed.resolve():
                    paths[name].write_bytes(committed.read_bytes())
        elif out.resolve() != HERE.resolve():
            # A disposable primary-comparison smoke run has no property artifacts to reuse.
            # Empty, schema-bearing files keep its manifest complete without pretending the
            # statistical study ran.  Publication to HERE still refuses this state.
            from tests.studies.evidence.properties import REPLICATE_COLUMNS as PROPERTY_COLUMNS

            write_csv(
                pd.DataFrame(columns=PROPERTY_COLUMNS),
    ...
    write_manifest(
        ...
        reference_files=[HERE / "Dockerfile", HERE / "run_study.R"],
        reference_metadata=REFERENCE_METADATA,
        configuration=CONFIGURATION,
    )

    property_summary = pd.read_csv(paths["properties.csv"])
    property_failures = (
        property_summary.query("not passed") if not property_summary.empty else property_summary
    )
    failures = {
        "independent performance": performance.loc[~performance["passed"]],
        "paired similarity and non-inferiority": paired.loc[~paired["passed"]],
        "reference validity": paired.loc[~paired["reference_valid"]],
        "statistical property": property_failures,
    }
