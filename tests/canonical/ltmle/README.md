...
This directory freezes one end-of-study fit and one survival fit from R `ltmle` 1.3-0.
...
Both fixtures use fixed numeric g predictions, intercept-only Q regressions, no
...
1.3.0. The generated artifacts are `longitudinal.csv`, `survival.csv`, and
`reference.csv`; `tests/unit/test_ltmle_canonical_r.py` consumes them without requiring R
or Docker in CI.
