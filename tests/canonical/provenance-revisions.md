...
| --- | --- | --- | --- |
| `tests/canonical/lmtp_crossfit_adapter.R` | `788af2d43a3489597231e271769eca5d2b0b15e786be8482233819fe0e023a00` | `6ea0d48d3143222f0342ea49fde84affa0740ba668fbdb874f5f0919d31e97b6` | result-neutral: every existing study passes no identifier and no weights. Three use five labels. The ordinary competing row uses one label, and it reaches its folds through `lmtp_competing_adapter.R`, which builds the single-fold case itself and never calls `fold_list`. Each one declares a binomial outcome, or a survival outcome above horizon one, and the continuous-initial branch tests `outcome_type == "continuous"`, which neither value is. The identifier, weight, cluster-integrity, and continuous-initial branches do not run for those recorded studies. The later edit replaces a cumulative-mean validation screen with a first-node screen before assigning the same supplied matrix; it can refuse a future invalid input but cannot alter a successful fit's input or numeric result |
| `tests/canonical/lmtp_crossfit_adapter.R` | `9ec0c76f9ed1ee41276b73071d51baaacae8537662e2f61994ac49cc4493b71f` | `6ea0d48d3143222f0342ea49fde84affa0740ba668fbdb874f5f0919d31e97b6` | result-neutral: the categorical studies pass no identifier and no weights, and they have a binary outcome. The identifier, weight, cluster-integrity, and continuous-initial branches do not run for those recorded studies. The later edit replaces a cumulative-mean validation screen with a first-node screen before assigning the same supplied matrix; it can refuse a future invalid input but cannot alter a successful fit's input or numeric result |
| `tests/canonical/lmtp_crossfit_adapter.R` | `7f5d2b663e4dca986ade973a8d263ea132a666ff8d59444ecaa407f607b0f8dc` | `6ea0d48d3143222f0342ea49fde84affa0740ba668fbdb874f5f0919d31e97b6` | result-neutral: the supplied-ratio guard now tests the expectation-one property at the first node, where it holds, instead of on a cumulative product that can be structurally zero after an event. The guard runs before assigning the supplied matrix; it changes refusal behavior only and leaves every successful fit's numeric input unchanged. The ordinary, weighted, clustered, and competing mutation smokes pass the shared guard |
| `tests/canonical/lmtp_competing_adapter.R` | `c4f5d807b1810a77baf408ff65322958f8dcbae68620ec64bd7cbe98f4a30f01` | `4dc2ae000afdf45bc1bc3511a08e0f02fe3f9969cf7bc948cefbe3c35f80d417` | result-neutral: the competing adapter deletes its local supplied-ratio guard and uses the shared guard sourced immediately before it. The moved logic retains the competing adapter's first-node five-standard-error screen and every structural and correlation check. It validates before assigning the same ratio matrix and cannot change a successful fit's numeric input or result; the competing mutation smoke passes both valid fold configurations and rejects opposite-arm and dropped-censoring inputs |
...
| `omitted-variable-bound-se` | `tests/studies/omitted_variable_bound.py` | `8184e8e1aba9445ba5943adb15840bb29de5649b7fab644f0d3a7f6f5c33d862` | `c1a734411d2449da09607dd99f4be23e88c230070191f4d80b6103bdfbfdb311` | result-neutral: commit f0de42f added assertions that the rebuilt bias curve plus the share term equals the package's `psi_max_bias` and reported standard errors within 1e-10, and that the ATE control curve equals the package's curve. It also removed a blanket warning filter from `fit()`. The assertions raise or pass and change no value, and the declared fit raises no warning. With warnings raised as errors, the first five primary and five property replications refit to a maximum difference of 0.0 in every estimate, standard error and interval endpoint against the committed artifacts, and every rebuild assertion passes |
| `omitted-variable-bound-se` | `tests/studies/omitted_variable_bound_properties.py` | `9f900e4bc9f86137810912b86d6d60c0a5d2d8ea5f643b1b49a5a8ceac04eb7b` | `f566666078903718711c65b0fcd093c7a13c00dd0ec2353569843979479f1cb2` | result-neutral: the run read a CRLF working copy of this file, although `.gitattributes` sets `eol=lf`. The recorded hash is the sha256 of the committed LF blob with each line feed written as a carriage return and a line feed, byte for byte. The content is otherwise identical, and Python reads both files as the same source |
| `omitted-variable-bound-se` | `tests/studies/evidence/property_verdicts.py` | `335c61d3bed687eb74a4d2d3e37c8c5dea1d78f67bdf5cc84c8cdfb57ce35822` | `c8a60ed7a10a9f151586da5e112235c2b844312d3e888f2f4304a104bb0aa437` | result-neutral: the same cause. The run read a CRLF working copy, and the recorded hash is the sha256 of the committed LF blob written with CRLF line endings. The content is otherwise identical |
