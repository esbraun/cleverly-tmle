...
| --- | --- | --- | --- |
| `stacked-mar-natural-course-cvtmle` | `configuration.comparator_search` | the recorded text rejects `tmle3` because no maintained callable task was identified, and zEpid because it targets treatment contrasts. The recorded text also rejects R `tmle` because its MAR API reports intervention-arm means. `CONFIGURATION` in `tests/studies/canonical_mar_natural_course_cvtmle.py` now gives the reasons whose source locators `docs/references.md` records: the `tmle3` generic outcome fit can use the `Delta = 0` pseudo-outcomes, zEpid targets inside each fold, and Newey and Robins (2018) is a different estimator. It states that R `tmle` was not compared, because its population-mean path has not been run on fold predictions | result-neutral: the string is copied into the manifest and read by no fit, summary, or verdict. The study has no reference implementation, so `equivalence.csv` has zero rows under either text |
